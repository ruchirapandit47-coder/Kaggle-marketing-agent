...
def creative_studio_agent(node_input: dict, ctx: Context) -> Event:
    ...
    # DEBUG STATEMENTS
    print(f"DEBUG CS_OUTPUT: {str(output_obj).encode('ascii', errors='replace').decode('ascii')}", flush=True)
    print(f"DEBUG CS_TYPE: {type(output_obj)}", flush=True)
    print(f"DEBUG CS_KEYS: {list(output_obj.model_fields.keys())}", flush=True)
