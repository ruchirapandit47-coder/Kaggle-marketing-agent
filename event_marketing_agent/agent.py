...
def prepare_route_input(node_input, ctx: Context) -> Event:
    ...
    try:
        # DEBUG STATEMENTS
        print(f"DEBUG STORE INPUT: {str(node_input).encode('ascii', errors='replace').decode('ascii')}", flush=True)
        ...
        print(f"DEBUG ctx.state['creative_assets']: {str(ctx.state['creative_assets']).encode('ascii', errors='replace').decode('ascii')}", flush=True)
...
def generate_final_report(node_input, ctx: Context) -> Event:
    ...
    try:
        ...
        # DEBUG STATEMENTS
        print(f"DEBUG REPORT creative_assets: {str(creative).encode('ascii', errors='replace').decode('ascii')}", flush=True)
        if isinstance(creative, dict):
            print(f"DEBUG REPORT KEYS: {list(creative.keys())}", flush=True)
        else:
            print(f"DEBUG REPORT KEYS (non-dict): {type(creative)}", flush=True)
